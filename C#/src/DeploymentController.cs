...
static class DeploymentController
{
    ...
    private Direction _currentDirection = Direction().UpDown ////may need new direction/shipname objects?
    private ShipName ShipName = ShipName().Tug
   ...
   /*  ''' <summary>
    ...
    private void DoDeployClick()
	{
        ...
        int row, column;
		////change convert.toint32() to c#'s int32.parse()
  ...
  /*   ''' <summary>
    ...
    public void DrawDeployment()
	{
        ...
        /* 'DrawShips */
		////convert below foreach statement to c# using enumorator
        foreach(Each sn As ShipName In [Enum].GetValues(GetType(ShipName)))
		{
			int i;
			////convert sn to int using c#
            i = Int(sn) - 1;
            if(i >= 0) {
				if(sn = _selectedShip{
    ...
    private ShipName GetShipMouseIsOver(){
		////convert below foreach statement to c# using enumorator
		foreach(sn As ShipName In [Enum].GetValues(GetType(ShipName)))
			int i;
			////convert sn to int using c#
            i = Int(sn) - 1;
